...
/// The kind of file we're working with.
#[derive(Debug, EnumIter)]
pub enum FileType {
    /// A project file (notes, instruments, etc.)
    Project,
    /// An instrument or effect configuration
    Patch,
    /// An audio file, possibly with metadata describing it
    Sample,
}
...
impl Paths {
    ...
    /// The directory containing assets like samples, patches, and demo
    /// projects.
    ...
    /// Returns the [PathBuf] to the given hive type
    ...
    /// Returns an array of all hive [PathBuf]s.
    ...
    /// Returns the directory containing projects installed with the
    /// application.
    #[deprecated]
    ...
    /// Finds the first matching file in the hives at the given relative path
    /// and reads it as a String.
    ...
    /// Constructs the right full path for the given file type, searches for it
    /// in the hives, and opens it.
    ...
    /// Builds a [PathBuf] for the given instrument key and patch name
    ...
    /// Builds a [PathBuf] for the given instrument key and sample name
...
pub(crate) mod tests {
    ...
    fn mainline_ok() {
        ...
        // We don't guarantee this number, but it's good to keep an eye on it in
        // case it changes.
        if cfg!(unix) {
    ...
    fn precedence() {
        ...
        // (WINDOWS) think a newline is \r\n, and I'd prefer cross-platform
        // approaches.
